...
use ci_ext\validators\StringValidator;
...
class Html {
	...
	{
		return htmlspecialchars($text,ENT_QUOTES,\CI_Ext::charset());
	}
	...
	public static function encodeArray($data)
	{
		...
		{
			if(is_string($key))
				$key=htmlspecialchars($key,ENT_QUOTES,\CI_Ext::charset());
			if(is_string($value))
				$value=htmlspecialchars($value,ENT_QUOTES,\CI_Ext::charset());
	...
	{
		return '<input type="hidden" name="page_state" value="'.$value.'" />';
	}
...
EOD;
	...
	public static function ajax($options)
	{
		if(!isset($options['url']))
			$options['url']=new JavaScriptExpression('location.href');
		else
			$options['url']=self::normalizeUrl($options['url']);
		if(!isset($options['cache']))
			$options['cache']=false;
		if(!isset($options['data']) && isset($options['type']))
			$options['data']=new JavaScriptExpression('jQuery(this).parents("form").serialize()');
		foreach(array('beforeSend','complete','error','success') as $name)
		{
			if(isset($options[$name]) && !($options[$name] instanceof JavaScriptExpression))
				$options[$name]=new JavaScriptExpression($options[$name]);
		}
		if(isset($options['update']))
		{
			if(!isset($options['success']))
				$options['success']=new JavaScriptExpression('function(html){jQuery("'.$options['update'].'").html(html)}');
			unset($options['update']);
		}
		if(isset($options['replace']))
		{
			if(!isset($options['success']))
				$options['success']=new JavaScriptExpression('function(html){jQuery("'.$options['replace'].'").replaceWith(html)}');
			unset($options['replace']);
		}
		return 'jQuery.ajax('.JavaScript::encode($options).');';
	}
	...
	{
		return get_instance()->publish($path,$hashByName);
	}
	...
	public static function normalizeUrl($url)
	{
		if(is_array($url))
		{
			if(isset($url[0]))
			{
				$url=get_instance()->createUrl($url[0],array_splice($url,1));
			}
			else
				$url='';
		}
	...
	protected static function clientChange($event,&$htmlOptions)
	{
		...
		{
			$confirm='confirm(\''.JavaScript::quote($htmlOptions['confirm']).'\')';
			if($handler!=='')
		...
		if($live)
			get_instance()->registerScript('CI_Ext.Html.#' . $id, "$('body').on('$event','#$id',function(){{$handler}});");
		else
			get_instance()->registerScript('CI_Ext.Html.#' . $id, "$('#$id').on('$event', function(){{$handler}});");
